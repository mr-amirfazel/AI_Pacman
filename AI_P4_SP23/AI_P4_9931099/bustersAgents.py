...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghost_posit in ghost_positions:
            # find distance to each ghost
            pacman_ghost_dist = self.distancer.getDistance(pacmanPosition, ghost_posit)
            for action in legal:
                succ_pos = Actions.getSuccessor(pacmanPosition, action)
                temp_dist = self.distancer.getDistance(succ_pos, ghost_posit)
                if pacman_ghost_dist > temp_dist:
                    finalAction = action
                    pacman_ghost_dist = temp_dist
            return finalAction
