                ...
                for a in actions:
                    ...
                    for stateProb in transitionStatesAndProbs:
                        sumValue += stateProb[1] * (
                                self.mdp.getReward(s, a, stateProb[0]) + self.discount * self.values[
                            stateProb[0]])
    ...
    def runValueIteration(self):
        ...
        for it in range(self.iterations):
            ...
            if self.mdp.isTerminal(s):
                continue
            state_one = util.Counter()
            for a in self.mdp.getPossibleActions(s):
                state_one[a] = self.computeQValueFromValues(s, a)
            self.values[s] = max(state_one.values())
