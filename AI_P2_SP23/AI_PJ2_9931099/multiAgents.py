...
def betterEvaluationFunction(currentGameState):
    ...
    PowerPillCount = len(currentGameState.getCapsules())

    score = 0
    emptyFoodCount = len(newFood.asList(False))
    scaredCount = sum(newScaredTimes)
    ghostDistanceSum = sum(ghostDistance)
    foodDistanceInvert = 0
    if sum(foodDistance) > 0:
        foodDistanceInvert = 1.0 / sum(foodDistance)

    score += currentGameState.getScore() + foodDistanceInvert + emptyFoodCount

    if scaredCount > 0:
        score += scaredCount + (-1 * (PowerPillCount + ghostDistanceSum))
    else:
        score += ghostDistanceSum + PowerPillCount
    return score
